...
create_backup() {
    ...
    if sudo docker exec n8n-postgres pg_dump -U n8n n8n > "$backup_dir/database.sql"; then
        log_success "✅ Database backup thành công"
    else
        log_error "❌ Database backup thất bại"
        return 1
    fi
    
    # 2. Backup n8n data files
    log_info "📁 Backup n8n data files..."
    ...
    if [[ -n "$n8n_volume" ]]; then
        ...
        log_success "✅ Data files backup thành công"
    else
        log_error "❌ Không tìm thấy n8n data volume"
        return 1
    fi
    
    # 3. Backup docker-compose và config
    log_info "⚙️ Backup cấu hình..."
...
EOF
    ...
    log_info "🗜️ Đang nén backup..."
    cd "$BACKUP_BASE_DIR"
    sudo tar -czf "${backup_name}.tar.gz" "$backup_name"
    sudo rm -rf "$backup_name"
    
    log_success "✅ Backup hoàn tất: ${backup_name}.tar.gz"
...
EOF
    ...
    # Add to crontab
    (crontab -l 2>/dev/null | grep -v "$CRON_JOB_NAME"; echo "$cron_schedule $cron_script # $CRON_JOB_NAME") | crontab -
